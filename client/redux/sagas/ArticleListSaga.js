...
export function * getAllListFlow () {
  while (true) {
    let req = yield take(ArticleActionTypes.GET_All_LIST)
    let res = yield call(get, api.getAllList())

    if (res.retCode === 1) {
      yield put({
        type: ArticleActionTypes.RESPONSE_All_LIST,
        data: res.data
      })
    }
  }
